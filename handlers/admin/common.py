...
def _logs_menu_rows():
    """Три ветки экрана «📜 Логи бота» (2026-08-16, третья — 2026-09-21).
    ...
    Третья ветка (21.09.2026, просьба Максима) — прямые обращения к боту:
    личка и обращения в группе, запись на каждого человека отдельно.
    """
    # Первые две ветки в ОДИН ряд, два столбца (решение Максима 2026-08-16):
    # выбор из равнозначных веток читается как выбор, а не как список. Третья
    # стоит своей строкой: она ведёт не к тексту лога, а к списку людей.
    return [
        [
        ...
        [InlineKeyboardButton("👤 Разговор с ботом", callback_data="adm_logs_dlg")],
    ]
...
def _build_logs_menu_text() -> str:
    ...
    people = dialog_log.count_records()
    dlg_line = (f"Записей: {people} чел. · {human_size(dialog_log.total_size())}"
                if people else "Записей пока нет — боту никто не писал напрямую")
    ...
    return (
        ...
        "<i>Прямые обращения — личка и обращения в группе. Запись на каждого "
        "человека своя.</i>\n"
        f"{dlg_line}\n"
        "───────────────────────────\n"
