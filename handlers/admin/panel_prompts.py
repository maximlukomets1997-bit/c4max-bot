...
def _build_prompt_panel_text_and_keyboard(user_id, bot_username=None):
    ...
    # ⚠️ СТРОКИ ПРО ПОСЛЕДНЮЮ НОВОСТЬ ЗДЕСЬ БОЛЬШЕ НЕТ (2026-08-20): из
    # проактивного запроса блок убран, и её размер в этой сводке был бы
    # враньём. В обычных ответах новость по-прежнему уходит — об этом
    # сказано строкой под списком.
    full_text += (
        "\n───────────────────────────\n"
        ...
        "<i>Последняя новость сюда БОЛЬШЕ НЕ уходит (20.08) — только в обычные ответы: упоминание бота в группе, личка, голосовые, видео.</i>"
    )
