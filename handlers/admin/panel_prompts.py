...
def _build_prompt_panel_text_and_keyboard(user_id, bot_username=None):
    ...
    # Последняя разосланная новость (2026-08-16): в постоянную часть НЕ входит —
    # её размер меняется с каждой рассылкой, а до первой новости её нет вовсе.
    try:
        from services.gemini import last_news_brief
        news_len = len(last_news_brief() or "")
    except Exception as e:
        logger.debug("🔧 Не удалось измерить блок последней новости: %s", e)
        news_len = 0
    news_line = (f"{_num(news_len)} <i>симв.</i>" if news_len
                 else "<i>рассылок ещё не было</i>")
    full_text += (
        "\n───────────────────────────\n"
        ...
        f"3. 📰 Последняя новость — {news_line}\n"
