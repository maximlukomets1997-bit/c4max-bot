...
def _proactive_stats_lines() -> str:
    """
    Строка итогов за сутки для панели промптов. Держать КОРОТКОЙ: текст
    ...
    ⚠️ Строка отсева («Отсеяно до модели: N») убрана отсюда 2026-08-11 по
    решению Максима. Сами числа НЕ пропали — они на экране «📊 Подробнее»,
    с разбивкой по причинам. Не возвращать сюда без его просьбы.
    """
    try:
...
async def _handle_proactive_wipe(query, user_id: int, confirmed: bool, from_adm: bool = False):
    ...
    Из панели промптов действие идёт через подтверждение: клавиатура сначала
    подменяется на «да / отмена» — тем же приёмом, что очистка журнала в
    панели модерации.

    from_adm=True — нажали дубликат кнопки в /adm. Там подтверждения нет
    (просьба Максима 2026-08-10), сюда приходят сразу с confirmed=True, и
    клавиатуру после очистки НЕ трогают: она не менялась, а повторная отправка
    той же разметки — это ошибка Telegram «Message is not modified» в логах.
    """
    ...
    # Возвращаем кнопки панели промптов: перед очисткой там стояло
    # подтверждение «да / отмена», и его надо сменить обратно на обычные.
    #
    # ⚠️ ИЗ /adm КЛАВИАТУРУ НЕ ТРОГАЕМ ВОВСЕ (2026-08-11). С тех пор как
    # подтверждение в админ-панели убрали, её кнопки при нажатии НЕ менялись —
    # и «возврат» отправлял Телеграму ровно ту же разметку, что уже на экране.
    # Telegram на это отвечает ошибкой «Message is not modified», она падала в
    # лог отладочной строкой и выглядела как поломка (её и заметил Максим).
    # Поломки не было — был лишний запрос к Telegram на каждое нажатие.
    if not from_adm:
        try:
            _, markup = _build_prompt_panel_text_and_keyboard(user_id)
            await query.edit_message_reply_markup(reply_markup=markup)
...
# ─────────────────────────────────────────────
#
#  У панели промптов ПЯТЬ превью (система, дополнения, новости, RAG, участие)
#  по 600 символов каждое — в сумме 3000 при постоянной части
...
def _build_prompt_panel_text_and_keyboard(user_id, bot_username=None):
    ...
    # ⚠️ СТРОКИ ПРО ПОСЛЕДНЮЮ НОВОСТЬ ЗДЕСЬ БОЛЬШЕ НЕТ (2026-08-20): память
    # о последней новости удалена целиком, отдельным блоком она не уходит
    # никуда. Сводка ложится в переписку получателя при отправке и живёт
    # там, пока не уедет из окна контекста — как любое другое сообщение.
    full_text += (
        "\n───────────────────────────\n"
        ...
        f"3. 🗣 PROMPT участия — {_num(len(proactive_instruction))} <i>симв.</i>\n"
        f"4. 🪪 Справка об авторе — {_num(who_len)} <i>симв.</i>\n"
        f"5. 💬 Стенограмма чата — <i>последние {proactive_ctx} сообщ.</i>\n"
        f"<i>Постоянная часть (1+3+4): {fixed_total} символов</i>\n"
        "<i>Пункты 1–4 уходят системным текстом, стенограмма — отдельным "
        "сообщением от лица человека.</i>\n"
        "<i>Последняя новость отдельным блоком НЕ уходит никуда (20.08): сводка ложится в переписку получателя при отправке и живёт там, пока не уедет из окна контекста.</i>"
    )
    ...
    # Полная раскладка — на экране «📊 Подробнее», здесь только одна строка:
...
#
#  ⚠️ Новый промпт заводится ОДНОЙ записью в _PROMPTS плюс парой тонких
#  обёрток ниже — и не забыть зарегистрировать команды в handlers/__init__.py,
#  добавить их в re-export handlers/admin/__init__.py и дописать пару его
#  callback'ов в список литералов роутера (вычисляемый там не годится).
#  Ровно так 2026-08-05 заводился пятый промпт (разбор медиа) и ровно так же
#  2026-08-11 разбирался обратно по решению Максима.
