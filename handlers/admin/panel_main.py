...
# 2026-08-17: добавлены gemini-3.7-flash и qwen3.8-max — стало 6 рядов.
# ДВЕ НОВЫЕ СТОЯТ ПЕРВЫМ РЯДОМ ВМЕСТЕ (решение Максима в тот же день, он же
# отменил первоначальную раскладку «строго парами по провайдерам»). Из-за этого
# первый ряд — ЕДИНСТВЕННЫЙ, где рядом стоят разные провайдеры: Gemini и Qwen.
# Так и задумано — ряд читается как «новинки», а не как «модели одного сервиса».
# Ниже июльские пары Gemini вернулись на прежние места, в порядке «по столбцам»,
# который задавал Максим 2026-07-24.
_MODEL_BUTTON_ROWS = [
