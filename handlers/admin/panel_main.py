...
# 2026-08-17: добавлены gemini-3.7-flash и qwen3.8-max — стало 7 рядов.
# КАЖДАЯ НОВАЯ — СВОЕЙ СТРОКОЙ, В КОНЦЕ БЛОКА СВОЕГО ПРОВАЙДЕРА (решение
# Максима в тот же день; до этого пробовали «строго парами» и «новинки вместе
# первым рядом» — обе раскладки он отменил). Ряд из одной кнопки Telegram
# растягивает на всю ширину, и это осознанно: длинная кнопка читается как
# «самая новая». Правило «ряд = один провайдер» снова соблюдается везде.
# Июльские пары Gemini стоят в порядке «по столбцам», заданном 2026-07-24.
_MODEL_BUTTON_ROWS = [
