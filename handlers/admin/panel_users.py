...
_ACTION_TITLES = {
    ...
    # Стирание ДОСЛОВНОЙ ЗАПИСИ обращений (21.09.2026) — это не память бота,
    # а лог logs/dialog. Код отдельный от clear_ctx намеренно: очистка диалога
    # меняет то, что бот помнит, а это — только то, что можно перечитать.
    "dialog_clear": ("👤", "очистка записи обращений"),
    ...
    # Чужие группы (2026-09-15): кнопки «✅ Остаться» и «🚪 Выйти» под вопросом
    # «меня добавили в группу» (services/group_guard.py). Что именно решено,
    # сказано в подробности записи («вышел из группы «Ветеэм»»).
    "group":      ("🚪", "чужая группа"),
