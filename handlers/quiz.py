...
async def send_quiz_question(chat_id: int, context: ContextTypes.DEFAULT_TYPE,
    ...
    auto=True — это ВОПРОС ДНЯ (2026-08-20, просьба Максима): по расписанию
    из config.QUIZ_AUTO_HOURS (12:00 и 18:00 по Киеву), см.
    services/quiz_daily.py. Отличий ровно два:
      • НЕ ставится авто-удаление через минуту — вопрос висит до следующего
        срока, чтобы люди успели ответить; предыдущий убирает эта же отправка;
    ...
    try:
        ...
        # его увидит только тот, кто смотрел в чат ровно в срок. Предыдущий
        # опрос убирает следующая отправка по расписанию (jobs/reports.py).
        if not auto:
