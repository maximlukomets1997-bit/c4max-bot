...
async def daily_quiz(application) -> bool:
    ...
    ⚠️ Группы берём из known_chats — там только СВОИ группы: с 15.09.2026 туда
    вносит группу одобрение владельца (services/group_guard.py), а группа,
    откуда бота удалили, из списка уходит сама. Подписка на новости тут ни при чём: на неё
    подписаны и личные чаты, а вопрос дня — групповая затея.
