...
async def sync_balances_once() -> None:
    ...
    for provider, ask in _providers().items():
        ...
        # «совпало» в лог не пишем: это рутина каждого часа, а не событие.


async def balance_sync_loop(application):
    """
    Раз в час спрашивает у платформы настоящий остаток на счету.
    ...
    экраны целый час показывали бы собственную оценку.
