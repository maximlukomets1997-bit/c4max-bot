...
async def drop_expired_notice(application) -> None:
    ...
    через 10 минут»; в тот же день срок сокращён до пяти — живое значение
    всегда в config.UPDATE_NOTICE_TTL_SEC, здесь числа не дублируем).
    ...
    Зовут её двое, и оба нужны:
      ...
      • main.post_init при запуске — если бот простоял выключенным дольше
        своего срока, уведомление устарело ещё до его пробуждения.
