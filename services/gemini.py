...
#    бот автоматически переключается на FALLBACK_MODEL (имя — только из
#    config.py, здесь оно не дублируется: зашитая строка переживает смену
#    модели и начинает врать) и пробует её. Только если недоступно вообще всё — отдаётся мягкое
...
def _notify_models_failed(kind: str, failures: list,
    ...
    дважды, в цепочке до пяти моделей (11.09.2026: было четыре, первым звеном
    очередей встала 3.8 Flash) — без этого правила один сбойный вопрос давал
    бы до шести сообщений подряд.
    """
    if not failures:
        return
    if used_model:
...
#  Чем ОБЕРНУЛСЯ провал для человека — последняя строка письма владельцу.
#  Провалы разные, и до 28.08.2026 письмо описывало их все одинаково, словами
#  проактивного режима: «бот промолчал». Для лички это было прямой неправдой —
#  см. _notify_chain_dead ниже.
_DEAD_SILENT = "Бот промолчал — в чате это выглядит как «проигнорировал сообщение»."
_DEAD_NO_KB = "На сообщение бот ОТВЕТИЛ, но без статей базы знаний — поиск по ней не состоялся."
_DEAD_STUB = "Человек получил сообщение-заглушку вместо ответа."


def _notify_chain_dead(what: str, failures: list,
                       consequence: str = _DEAD_SILENT) -> None:
    """
    Цепочка не справилась ни одной моделью (2026-08-18, просьба Максима
    «добавить сообщения про голосовые с видео на их полный провал»).

    Это самая тихая поломка бота: человек прислал голосовое или ролик, бот его
    молча не понял. Отдельные отказы внутри цепочки НЕ сообщаются (следующая
    модель подхватит, иначе сыпалось бы на каждую картинку) — только полный
    провал.

    ⚠️ ПИСЬМО ВРАЛО В ДВУХ МЕСТАХ (живой тест Максима 28.08.2026, видео в
    личке). Оба вранья — от того, что текст писался под ОДИН сценарий,
    ...
    2. «Бот промолчал» — а бот на то видео ОТВЕТИЛ, за двадцать секунд.
       ...
       врать оно больше не может, даже если появится четвёртый путь.

    consequence — что случилось С ЧЕЛОВЕКОМ: _DEAD_SILENT (бот промолчал,
    проактивный режим), _DEAD_NO_KB (ответил без базы знаний, поиск в личке),
    _DEAD_STUB (получил заглушку, прямой ответ в личке).
    """
    # Сколько моделей реально пробовалось: пустой список — значит все сидели
    # на скамейке и в запрос не пошла ни одна.
    if not failures:
        head = "не справилась ни одна модель"
        why = "Все модели цепочки сидят на скамейке после ошибки квоты (429)."
    elif len(failures) == 1:
        head = "не справилась единственная модель"
        why = f"Не ответила: {_failed_list(failures)}."
    else:
        head = f"не справилась ни одна модель из {len(failures)}"
        why = f"Не ответили: {_failed_list(failures)}."
    _notify_admins(f"🛑 {what}: {head}\n{why}\n{consequence}")
...
def thinking_level(provider: str) -> str:
    ...
    (значок провайдера + фаза луны) в панели
...
#  Сколько моделей пробует РАЗБОР ФОТО РАДИ ПОИСКА ПО БАЗЕ в личке (07.09.2026,
#  решение Максима). Была одна: человек ждёт ответа, и перебор всей очереди
#  стоил бы ему минут.
#
#  ⚠️ ПОЧЕМУ ТРИ, А НЕ ДВЕ — посчитано по боевому логу 07.09, авария Google.
#  Пришло 8 фото одной пачкой: у 3.6 Flash 503 и исчерпанная квота, у 3.7 Flash
#  те же 503, владельцу ушло 8 писем подряд. Спасала в тот вечер ТРЕТЬЯ модель
#  очереди — в 18:32 из группы (там перебор полный) 3.5 Flash описала фото за
#  2.3 секунды после отказов обеих первых. Одна подстраховка упёрлась бы в
#  3.7 Flash, самое ненадёжное звено очереди: по замерам в config.py она
#  отвечала раз из семи.
#
#  ⚠️ 11.09.2026 СОСТАВ ЭТОЙ ТРОЙКИ ИЗМЕНИЛСЯ, а число осталось прежним: первым
#  звеном очередей встала 3.8 Flash, и теперь фото в личке пробует 3.8 → 3.6 →
#  3.7. Та самая 3.5 Flash, что спасла разбор 07.09, съехала на четвёртое
#  место и в тройку больше не попадает. Расклад стал такой: впереди две
#  думающие модели (3.8 и 3.7 нулевого уровня не принимают), между ними
#  надёжная 3.6. Понадобится вернуть 3.5 в тройку — двигать её в списках
#  config.py, а не это число.
#
#  ЦЕНА НАЗВАНА ВСЛУХ И ПРИНЯТА: когда модели не отказывают, а МОЛЧАТ, человек
...
#  теперь (в скобках — сколько моделей успеет подстраховать из пяти; до
#  11.09.2026 в очередях было четыре, потолки от этого не изменились — они
#  режут по времени, а не по числу звеньев):
#
...
#      максимум, тот же, что тогда стоял у обычных ответов бота).
#      ⚠️ ОБЕ ПОЛОВИНЫ ЭТОГО ПУНКТА С ТЕХ ПОР ОТМЕНЕНЫ, запись оставлена как
#      история решения: разбору мышление отключили 11.08 (см. ниже
#      `_MEDIA_THINKING_LEVEL`), а ответы бота 29.08 ушли с `thinking_budget`
#      на `thinkingLevel` под кнопку глубины. Сегодня у разбора и у ответов
#      РАЗНЫЕ уровни, и «тот же, что у ответов» читать здесь больше нельзя.
#
...
def _media_answer_thinking(model_name: str) -> dict:
    ...
    ⚠️ С 29.08.2026 уровень БЕРЁТСЯ ИЗ КНОПКИ «♊️ Gemini …», как у всех прочих
...
def _deepseek_chat_request(model_name: str, messages: list, thinking_override: bool | None = None):
    ...
    ⚠️ ШКАЛА: бот шлёт low / high / max (введены у DeepSeek 13.08.2026).
    ...
    ⚠️ ОБЕ ЗАМЕРЕННЫЕ МОДЕЛИ СНЯТЫ ПРОВАЙДЕРОМ 10.09.2026, работает V4.1 Flash;
    её ступени не замерялись, так что числа выше — про поведение шкалы вообще,
    а не про сегодняшнюю модель. Сама шкала у провайдера с того же дня шире:
    он принимает none, minimal, low, medium, high, xhigh, max (список выдан
    ответом 400 на заведомо неверное значение). Бот показывает не все —
    почему именно, написано у THINKING_LEVELS в config.py.

    ⚠️ ЦЕНА И ВРЕМЯ: рассуждения тарифицируются как обычный ответ, а он у
    DeepSeek с 2026-08-16 стоит по-разному в пик и вне пика (у V4.1 Flash —
    $1.20 против $0.60 за млн токенов). «Максимум» у снятой V4 Pro занимал
    92 секунды против 35 на «высокой»; у V4.1 Flash ступени не замерялись —
    ступень выбирает Максим кнопкой, в коде верхняя не зашивается. Точные
...
def _deepseek_peak_now() -> bool:
    """
    Пиковый ли тариф DeepSeek прямо сейчас: будний день (config.DEEPSEEK_PEAK_DAYS)
    И час внутри одного из окон (config.DEEPSEEK_PEAK_UTC). Условия ДВА, и
    нужны оба.

    ⚠️ ВЫХОДНЫЕ СЧИТАЮТСЯ ВНЕ ПИКА С 2026-08-23 — правило провайдера, а не наша
    выдумка; условие про день недели добавлено сюда 10.09.2026. До того бот
    смотрел только на час и в субботу-воскресенье завышал расход ровно вдвое
    в те же утренние окна. Деньги от этого не пропадали: счёт бота расходился
    с настоящим списанием DeepSeek, то есть врал в сторону «дороже».
    ⚠️ Час и день берутся В UTC, а не по Киеву: окна заданы провайдером в UTC
    и перевода часов не знают (см. оговорку у константы в config.py). У дня
    недели это важно вдвойне — под Новый год киевская суббота начинается,
    когда в UTC ещё пятница.
    ...
    if now.weekday() not in DEEPSEEK_PEAK_DAYS:
        return False
    return any(start <= now.hour < end for start, end in DEEPSEEK_PEAK_UTC)
...
def _native_thinking_config(model_name: str) -> dict:
    ...
    ⚠️ Уровень берётся из кнопки «♊️ Gemini …» (29.08.2026) и шлётся ВСЕГДА,
...
def _gemini_chat_request(messages: list, kind: str = "текст", has_image: bool = False,
    ...
    (generate_for_article) со значением True: сборка вопросов викторины — не
    ответ бота, и кнопке глубины раздумий она подчиняться НЕ должна
    (решение Максима 05.08.2026 — вопросы собираются на полную).
    ...
    if chain_override:
        ...
        # шустрее на замере). Порядок «запасная → lite» сохранён НАМЕРЕННО, а
        # не заменён на общий порядок медиа-очередей: здесь человек уже ждёт
        ...
        # ⚠️ 11.09.2026 сама `FALLBACK_MODEL` сменилась с 3.6 Flash на 3.8 Flash
        # (решение Максима, разбор в config.py у этой константы). ЗДЕСЬ это
