...
def _build_proactive_parts(chat_id: int, bot_id: int, trigger_text: str,
    ...
    # оно приходит сюда уже обёрнутым — «[на фото: …]» / «[на видео: …]»
    # (обёртку ставит services/proactive.py, там же написано почему: без неё
...
def ask_group_proactive(chat_id: int, bot_id: int, trigger_text: str,
    ...
    # с разбором фото («Вася: [на фото: …]») и не гадать по поведению бота.
    #
