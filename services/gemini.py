...
def _notify_chain_dead(what: str, failures: list,
    ...
    1. «не справилась НИ ОДНА модель» — а на пути поиска по базе в личке
       попытка ровно ОДНА и всегда была (chain_limit=1, чтобы человек не ждал).
       Читалось как «отказали все четыре», хотя отказала единственная. Теперь
       формулировка зависит от того, сколько моделей реально пробовалось.
...
def _describe_image(image_base64: str, chain_limit: int = 0,
    ...
    # базе (chain_limit=1) она всегда одна, и «не описала НИ ОДНА» читалось
    # как «отказали все четыре» — см. _notify_chain_dead.
