...
def _build_proactive_parts(chat_id: int, bot_id: int, trigger_text: str,
    ...
    # ⚠️ ПОСЛЕДНЕЙ НОВОСТИ ЗДЕСЬ БОЛЬШЕ НЕТ (убрана 2026-08-20, решение
    # Максима). Заводилась 16.08 «чтобы бот знал о своей рассылке везде», но в
    # проактивном режиме стоила дорого: полный текст сводки — 3.4 тысячи знаков,
    # примерно треть всего запроса, — уходил на КАЖДУЮ проверку в группе, пока
    # новость свежая (в среднем два дня, рекорд — 17).
    #
    # ⚠️ ЧЕМ ЗАПЛАТИЛИ: без обращения к себе бот больше не знает текста своей
    # рассылки. Пока она видна в стенограмме (окно последних сообщений) — видит
    # и он; уехала из окна, а люди её обсуждают — вклинится, не зная ни
    # заголовка, ни ссылки, ни условий. Ровно ради этого случая блок и заводили.
    #
    ...
    # своя не задана) — пустой кусок в промпт не кладём, иначе между ним
    # и справкой об авторе встала бы дыра из пустых строк.
    ...
    # ⚠️ СПРАВКА ОБ АВТОРЕ СТОИТ ЗДЕСЬ, ПОСЛЕДНЕЙ ПЕРЕД СТЕНОГРАММОЙ
    # (2026-08-20, решение Максима: «а то получается как-то разбросано»).
    # Читается по порядку: характер → факты из базы → правила участия →
    # кто говорит → сам разговор. До этого справка стояла между базой знаний
    # и правилами участия.
    # ⚠️ В обычных ответах (ask_gemini) порядок ДРУГОЙ и это нормально: там
    # промпта участия нет вовсе, и справка как шла сразу за базой знаний,
    # так и идёт.
