...
#    бот автоматически переключается на FALLBACK_MODEL (см. config.py; на
#    2026-08-17 это gemini-3.6-flash) и пробует её. Только если недоступно вообще всё — отдаётся мягкое
...
#
# ⚠️ ЗАМЕР 11.08 на боевом ключе (тогдашняя gemini-3.1-flash-lite, один вопрос;
# модель выведена из набора 17.08, но шкала уровней от этого не изменилась):
...
def _gemini_chat_request(messages: list, kind: str = "текст", has_image: bool = False,
    ...
    if chain_override:
        ...
        # ⚠️ ПОРЯДОК БЕРЁТСЯ ИЗ AUDIO_FALLBACK_CHAIN, А НЕ ЗАДАН ЗДЕСЬ СПИСКОМ
        # (2026-08-17). До этого имена трёх моделей были вписаны в код прямо
        # тут, с припиской «меняешь в config — поправь и здесь». Предполётная
        # проверка такой список НЕ видит: она сверяет только цепочки из
        # конфига. Выведи модель из набора — и здесь молча осталось бы имя,
        # которого в боте уже нет. Теперь источник один, разъезжаться нечему.
        # Фильтр по vision оставлен: цепочка общая для аудио и фото, а «видит
        # картинки» — свойство модели.
        chain = []
        for cand in AUDIO_FALLBACK_CHAIN:
        ...
        # со всей 2.5-серией; её место заняла gemini-3.1-flash-lite.
        # ⚠️ 2026-08-17: 3.1 Flash-Lite выведена из набора, её место заняла
        # 3.5 Flash-Lite (та же роль — быстрая бесплатная подстраховка, втрое
        # шустрее на замере). Порядок «3.6 Flash → lite» сохранён НАМЕРЕННО, а
        # не заменён на общий 3.7 → 3.6 → 3.5 → lite: здесь человек уже ждёт
        # ответа, и ставить впереди самого долгого мыслителя набора значит
        # платить секундами ровно там, где они дороже всего. Захочешь строго по
        # общему порядку — меняется эта строка, но сначала перечитай абзац.
...
# ⚠️ ПОТОЛКОВ ДЛИНЫ У СТРОК СТЕНОГРАММЫ БОЛЬШЕ НЕТ (решение Максима
# 2026-08-16). Были два: 300 знаков на чужую реплику и 2000 на свою — второй
# завели в тот же день, потому что под общие 300 попадала рассылка новости и
# обрывалась на первом абзаце. Теперь каждое сообщение уходит модели целиком,
# как его написали. Потолок сверху остался только один, и он не наш: Telegram
# не пропускает сообщения длиннее 4096 знаков.
#
# Цена решения: длинная простыня будет уходить в КАЖДОЙ проверке, пока не
# уедет из окна стенограммы (регулятор «📜 контекст»). Станет дорого — резать
# надо здесь и осознанно, а не подкручивать окно.
...
def _is_proactive_skip(body: str) -> bool:
    ...
    Ответить ровно словом-маркером (PROACTIVE_SKIP_MARKER) модель просит
    инструкция участия — с 2026-08-16 это единственное место, где о маркере
    сказано (зашитого задания в запросе больше нет),
...
def _last_news_block(for_log: bool = False) -> str:
    """
    ...
    for_log=True — ОДНА СТРОКА вместо всего блока, для лога разговора
    (2026-08-20, форму задал Максим). Модели по-прежнему уходит блок целиком:
    режется только то, что пишется в файл.
    ...
    # ⚠️ В ЛОГ РАЗГОВОРА — ОДНОЙ СТРОКОЙ (2026-08-20, просьба Максима).
    # Полный текст новости занимал 43–47% всей записи и повторялся в КАЖДОЙ
    # проверке, пока новость свежая: замер по живому логу сервера — 88 КБ
    # на семь проверок. По ссылке видно, что именно бот держал в голове, а
    # сам текст Максим и так видел, когда бот его рассылал.
    # ⚠️ Модели блок по-прежнему уходит ЦЕЛИКОМ: режется только файл.
    if for_log:
        # Ссылки может не быть (новость без адреса) — тогда заголовок.
        mark = news.get("url") or news.get("title") or "без ссылки"
        # Длина ВСЕГО блока, считая переводы строк между его строками:
        # строка стоит вместо него целиком, значит и мерить надо целиком.
        size = sum(len(x) for x in lines) + len(lines) - 1
        return f"[Последняя новость: {mark} — {size} симв.]"
    return "\n".join(lines)
...
def _build_proactive_parts(chat_id: int, bot_id: int, trigger_text: str,
    ...
    Возвращает ТРИ значения: (куски системной части, стенограмма отдельной
    строкой, куски для лога разговора).

    ⚠️ СТЕНОГРАММА ОТДЕЛЕНА ОТ СИСТЕМНОЙ ЧАСТИ (2026-08-16, решение Максима):
    она уходит модели сообщением «от лица человека», а не системным текстом —
    см. ask_group_proactive. Причина в проверке живыми запросами: без единого
    сообщения от человека Gemini отбивает запрос ошибкой 400, а модели Gemini
    стоят в цепочке подстраховки у любой активной модели.

    Куски для лога идут в прежнем порядке и включают стенограмму последней:
    запись показывает всё, что ушло, целиком. Отличие только в том, что
    заданные тексты промптов в ней заменены строкой с длиной.

    Все три — None, если говорить не о чем (пустая стенограмма, например
    сразу после кнопки «Очистить РАЗГОВОРЫ»).
    """
    ...
    for i, r in enumerate(rows):
        ...
        if not text:
            continue
        # ⚠️ НИКАКОЙ ОБРЕЗКИ ЗДЕСЬ БОЛЬШЕ НЕТ (2026-08-16, решение Максима
        # «убрать резку чужих и своих, чтобы не резались вовсе»). Каждая
        # реплика уходит модели целиком: и чужая, и своя, и разбор медиа.
        #
        # Как было и почему убрано: чужие строки резались на 300 знаках, свои —
        # на 2000 (второй потолок завели в тот же день, потому что под общие
        # 300 попадала рассылка новости и обрывалась на первом абзаце), а
        # разборы медиа не резались с 10.08 — обрезанный разбор превращался в
        # «игрок управляет колесной бронетехникой, машина начинает движение
        # в парковой зоне среди деревьев сакуры, лавочек и…», и модель не
        # узнавала ни исхода боя, ни техники противника. Теперь этот довод
        # распространён на всю стенограмму.
        #
        # ⚠️ Цена: длинное сообщение уходит в промпт на КАЖДОЙ проверке, пока
        # живёт в окне стенограммы. Сверху всё ограничено только пределом
        # самого Telegram — 4096 знаков на сообщение. Станет дорого — уменьшать
        # надо окно (регулятор «📜 контекст») или возвращать потолок осознанно.
    ...
    news_block = _last_news_block()
    if news_block:
        # Второй вызов перечитывает ту же запись из базы — это дешёвое чтение
        # settings, зато обе версии блока собраны одним кодом и разъехаться
        # не могут.
        _part(news_block, _last_news_block(for_log=True))
    ...
    # об авторе и стенограммой встанет дыра из пустых строк.
    # ⚠️ С 2026-08-16 это ЕДИНСТВЕННОЕ место, где модели объясняют правила игры,
    # включая слово ПРОПУСК: зашитого задания в запросе больше нет. Инструкция
    # пуста — модель не знает, что может промолчать, и ответит всегда.
    ...
    #
    # ⚠️ В system_parts стенограмма НЕ КЛАДЁТСЯ — она уходит отдельным
    # сообщением «от лица человека» (см. докстринг). В лог пишется как раньше,
    # последним куском: запись показывает весь запрос целиком.
    transcript = "[Контекст сообщений всех участников чата]\n" + "\n".join(lines)
    log_parts.append(transcript)
    return system_parts, transcript, log_parts
...
def ask_group_proactive_media(chat_id: int, bot_id: int, trigger_text: str,
    ...
    """
    system_parts, transcript, log_parts = _build_proactive_parts(
        chat_id, bot_id, trigger_text, trigger_user_id)
    ...
    if any(system_parts):
        base["systemInstruction"] = {"parts": [{"text": "\n\n".join(system_parts)}]}
...
def ask_group_proactive(chat_id: int, bot_id: int, trigger_text: str,
    ...
    В модель уходит ДВА сообщения. Системное: характер и должностная
    инструкция (системный промпт) + RAG-контекст по последнему сообщению +
    справка об авторе + инструкция участия (get_proactive_instruction; блок
    рук и слово ПРОПУСК в ней только если владелец вписал их сам). И следом
    сообщение «от лица человека» — стенограмма чата одним блоком.
    ...
    # ⚠️ СТЕНОГРАММА ИДЁТ СООБЩЕНИЕМ «ОТ ЛИЦА ЧЕЛОВЕКА» (2026-08-16, решение
    # Максима), а зашитого задания «вступить или ПРОПУСК» здесь больше нет:
    # ровно то же самое уже написано в конце инструкции участия, которую
    # владелец правит сам. Две причины держать стенограмму именно тут:
    #   • без единого сообщения от человека Gemini отбивает запрос ошибкой 400
    #     (проверено живыми запросами 16.08; Qwen, DeepSeek и Xiaomi терпят), а
    #     модели Gemini стоят в цепочке подстраховки у любой активной модели;
    #   • переписка — это данные, а не правила бота, и её место не в системной
    #     части.
    # Системную часть не отправляем, если она пуста (все промпты стёрты):
    # пустой системный текст модели тоже могут отбить.
    messages = []
    if any(system_parts):
