...
# ⏱ Чем кончилась ПРОШЛАЯ проверка в этом чате (2026-08-11, разбор случая
# Максима: бот промолчал на фото, а через минуту ответил на текстовое
# сообщение так, будто ему только что прислали картинку).
#
# Причина была в стенограмме: разбор фото остаётся в ней и выглядит таким же
# свежим поводом, как новое сообщение. Модель выбирала повод поинтереснее —
# картинку — и отвечала на неё с опозданием на минуту.
#
# Теперь в блоке контекста появляется строка «Ты: уже видел, решил
# промолчать» — модель понимает, что решение по прошлым сообщениям принято.
# Флаг ставит services/proactive.py через note_proactive_outcome: импорт в
# обратную сторону завёл бы кольцо (proactive уже импортирует gemini).
# В памяти процесса, как счётчики антиспама: перезапуск обнуляет.
_silent_last: set[int] = set()


def note_proactive_outcome(chat_id: int, silent: bool) -> None:
    """Запомнить, чем кончилась проверка в чате: молчанием или репликой."""
    if silent:
        _silent_last.add(chat_id)
    else:
        _silent_last.discard(chat_id)
...
def _build_proactive_parts(chat_id: int, bot_id: int, trigger_text: str,
    ...
    # понимала, ради чего её позвали: отвечала на разбор фото минутной
    # давности вместо нового сообщения — «как будто я ему фото прислал».
    # Теперь видно, что контекст, а что повод.
    #
    # Строку «Ты: уже видел, решил промолчать» ставим ТОЛЬКО если прошлая
    # проверка в этом чате кончилась молчанием: иначе модель возвращается к
    # отработанному поводу как к новости.
    if len(lines) > 1:
        context_lines = lines[:-1]
        if chat_id in _silent_last:
            context_lines.append("Ты: уже видел, решил промолчать.")
...
def ask_group_proactive(chat_id: int, bot_id: int, trigger_text: str,
    ...
    # ⚠️⚠️ ВРЕМЕННЫЙ ОТЛАДОЧНЫЙ ЛОГ — ВКЛЮЧЁН 2026-08-10 ПО ПРОСЬБЕ МАКСИМА
    # НА ВРЕМЯ ТЕСТА. Печатает ЦЕЛИКОМ всё, что уходит модели при проактивной
    # проверке: характер, RAG, справку об авторе, инструкцию участия и всю
    # стенограмму. Нужен, чтобы своими глазами увидеть, как выглядят строки
    # с разбором фото («Вася: [текст модели]») и не гадать по поведению бота.
    #
    # ⚠️ УБРАТЬ ПОСЛЕ ТЕСТА — это десятки строк в лог на КАЖДУЮ проверку в
    # КАЖДОЙ группе (а проверка идёт на каждое сообщение), архив логов пухнет
    # в разы и лог сессии становится нечитаемым. Удаляется этот блок целиком,
    # больше ничего трогать не нужно.
    _dbg = "\n\n".join(system_parts)
    logger.info(
        "🧪 ═══ ЧТО УХОДИТ МОДЕЛИ (проактив, чат %s) ═══\n"
        "───── СИСТЕМНАЯ ЧАСТЬ (%d символов) ─────\n%s\n"
        "───── ЗАДАНИЕ ─────\n%s\n"
        "🧪 ═══ конец запроса ═══",
        chat_id, len(_dbg), _dbg, task,
    )
