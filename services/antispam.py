...
async def _notify_moderators_mute(bot, chat_id: int, user_id: int, name: str,
    ...
    Шлём только тем модераторам, у кого есть право «🛡DDoS-Guard» (остальным
    кнопка всё равно не нажмётся). ВЛАДЕЛЬЦАМ не шлём: у них есть панель /mod
    с полным журналом, и лишний поток сообщений им не нужен — при желании
    добавить их сюда достаточно одной строки.
    ...
    try:
        ...
        recipients = [uid for uid in list_moderators() if can(uid, "mod")]
        if not recipients:
            return
