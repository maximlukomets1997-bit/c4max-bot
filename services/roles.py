...
_CALLBACK_RULES = (
    ...
    # Недельный дайджест группы: экран, тумблер и отправка В ГРУППУ — только
    # владельцу (сообщение уходит всем участникам чата от имени бота).
    ("dig:",              "owner"),
    # История обновлений: экран про сам код бота — владельцу (в таблице явно,
    # чтобы уровень было видно, как у дайджеста).
    ("upd:",              "owner"),
    # Чужие группы (2026-09-15): «✅ Остаться» / «🚪 Выйти» под вопросом
    # «меня добавили в группу». Решает, где бот работает и тратит деньги, —
    # только владелец. Вписано явно, хотя запрет и так по умолчанию.
    ("grp:",              "owner"),
    ("mod:antispam:",     "owner"),
    ...
    # Счёт викторины — ТОЛЬКО владелец (04.09.2026, решение Максима). Это
    # «заработанное»: подмена счёта со стороны не видна никак, а звание при
    # этом меняется у всех на виду. Вписано явно, хотя запрет и так по
    # умолчанию, — чтобы уровень был виден в таблице рядом с соседями.
    # ⚠️ Приставка БЕЗ двоеточия — она одна покрывает все пять кнопок экрана:
    # usr:quiz:, usr:quizfix:, usr:quizset:, usr:quizzero:, usr:quizzerogo:.
    # С «usr:quiz:» под правило попала бы только первая, а остальные ушли бы
    # в запрет по умолчанию — тот же уровень, но уже молча и не видно в таблице.
    ("usr:quiz",          "owner"),
    ...
    # Дословные записи обращений к боту (21.09.2026) — ТОЛЬКО владелец: это
    # чужая переписка целиком, вместе с тем, что люди пишут боту в личку.
    # Запрет и так по умолчанию; вписано явно, чтобы уровень был виден в
    # таблице. ⚠️ Приставка «dlg:» одна покрывает все кнопки экрана:
    # dlg:card:, dlg:file:, dlg:clr:, dlg:clrgo:, dlg:wipe, dlg:wipe_yes/no.
    ("dlg:",              "owner"),
)
