...
#
#    • КАРТИНКИ СОБИРАЮТСЯ В ТОМ ЖЕ ПРОХОДЕ, что и текст, и помнят, из живой
#      секции они или из вырезанной. В рассылку уходит ОДНА карточка ТТХ,
#      поэтому список карточек упорядочен: сначала обычная карточка из живой
#      секции, потом премиум-карточка (prem_stat), и только потом — карточки
#      из вырезанных секций. Иначе в пост попадали характеристики техники,
#      о которой в тексте поста не сказано ни слова (жалоба Максима 2026-07-29).
#
...
#  Выбранные адреса пишутся в лог целиком — без них разбор «прислал не ту
#  картинку» упирается в то, что посмотреть выбор парсера просто негде.
# ───────────────────────────────────────────────
...
def _abs_url(src: str) -> str:
    ...
    # Ссылка вида «//static.wtmobile.com/…» (без схемы) — это УЖЕ чужой домен,
    # приклеивать к ней wtmobile.com нельзя: получался битый адрес
    # «https://wtmobile.com//static.wtmobile.com/…», и Telegram такую картинку
    # просто не скачивал.
    if src.startswith("//"):
        return f"https:{src}"
    if src.startswith("/"):
...
def _extract_article_body(soup: BeautifulSoup, max_chars: int = ARTICLE_MAX_CHARS):
    ...
    # Картинки статьи в порядке документа: (адрес, лежала ли в вырезанной секции).
    # Собираем их ЗДЕСЬ, а не отдельным проходом, — только так видно, к какой
    # секции картинка относится (см. разбор ниже, у сборки stat_images).
    pictures: list[tuple[str, bool]] = []
    ...
    for el in body.find_all(["h2", "h3", "p", "ul", "summary", "picture"]):
        ...
        # Картинка: текстом не становится, но запоминает, под вырезанной она
        # секцией или под живой. Проверка стоит ДО разбора классов текстовых
        # блоков — у <picture> их нет, и иначе она отсеялась бы как «чужая».
        if el.name == "picture":
            src = _picture_url(el)
            if src:
                pictures.append((src, skip_until_heading))
            continue
    ...
    # ⚠️ Ищем СНАЧАЛА внутри <main> (там же, где берётся заголовок статьи), и
    # только если не нашли — по всей странице, как было раньше. find() отдаёт
    # ПЕРВЫЙ подходящий элемент документа: подвал «предыдущая/следующая новость»
    # и прочие блоки вне <main> больше не могут подсунуть чужой баннер, а если
    # баннер лежит вне <main> — работает прежний путь, картинка не теряется.
    main_image = (_picture_url(main_el.find("picture", class_="keyvisual-outlet"))
                  or _picture_url(soup.find("picture", class_="keyvisual-outlet")))
    ...
    # Язык совпадает с версией страницы: на /ru/ это stat_ru.
    #
    # ⚠️ ПОЧЕМУ ЗДЕСЬ СОРТИРОВКА, А НЕ ПРОСТО ПОРЯДОК ДОКУМЕНТА. В рассылку
    # уходит ОДНА карточка — stat_images[:1] (jobs.py). Раньше картинки
    # собирались отдельным проходом по всему news-container, никак не связанным
    # с вырезанием текста, и первой могла оказаться карточка из секции, которую
    # парсер из текста ВЫБРОСИЛ («Премиумная версия…», «Способы получения…»).
    # Получалось ровно то, на что жаловался Максим: в посте характеристики
    # техники, о которой в самом посте не сказано ни слова.
    # Поэтому кандидаты раскладываются по трём корзинам, внутри каждой —
    # порядок документа:
    #   1) карточка из ЖИВОЙ секции и без «prem» в имени — то, что нужно;
    #   2) премиум-карточка (prem_stat) из живой секции;
    #   3) всё, что лежало в вырезанных секциях.
    # Ничего не выбрасываем — только переставляем: если разметка сайта окажется
    # иной, чем мы думаем, состав списка останется прежним, изменится порядок.
    plain, prem, dropped = [], [], []
    for src, was_cut in pictures:
        fname = src.rsplit("/", 1)[-1].lower()
        if "stat" not in fname:
            continue
        if was_cut:
            bucket = dropped
        elif "prem" in fname:
    ...
    stat_images = []
    for src in plain + prem + dropped:
        if src not in stat_images:
...
def fetch_article(url: str) -> dict:
    ...
    try:
        ...
        # ⚠️ Адреса картинок пишем В ЛОГ ЦЕЛИКОМ. Раньше строка сообщала только
        # «главная картинка: да» — и когда в рассылку уходило не то изображение,
        # по логам нельзя было установить ВООБЩЕ НИЧЕГО: какой адрес выбрал
        # парсер, было негде посмотреть. Теперь достаточно сверить адрес из лога
        # со статьёй на сайте.
        logger.info("📰 Статья разобрана: %d символов текста, карточек ТТХ: %d, главная картинка: %s",
                    len(text), len(stat_images), main_image or "нет")
        if stat_images:
            logger.info("📰 Карточки ТТХ статьи (в рассылку уйдёт первая): %s",
                        " | ".join(stat_images))
