...
# Ссылка markdown: [надпись](адрес). Адрес — без пробелов и скобок: так
# устроены все ссылки в статьях (YouTube, TikTok), а жадный разбор скобок
# ломался бы о текст вида «(см. ниже)» в соседнем предложении.
_MD_LINK_RE = re.compile(r"\[([^\]\n]+)\]\(\s*([^()\s]+?)\s*\)")


def _link_to_html(m) -> str:
    """
    Одна markdown-ссылка → <a href="…">надпись</a>.

    Адрес НЕ ПОХОЖ на ссылку (сноска «[1](#ref)», путь к файлу) — оставляем
    строку как была: Telegram принимает только http/https/tg, на остальном он
    отбивает всё сообщение целиком, и раздел просто не открылся бы.
    """
    label, url = m.group(1), m.group(2)
    if not re.match(r"(https?|tg)://", url, re.I):
        return m.group(0)
    # Кавычку в адресе экранировать нечем (html.escape зовётся с quote=False,
    # чтобы не портить кавычки в тексте статьи) — она порвала бы атрибут href.
    url = url.replace('"', "%22")
...
def _inline_html(text: str) -> str:
    ...
    ССЫЛКИ разбираем ПЕРВЫМИ, сразу после экранирования (2026-08-04): без
    этого `[📺 YouTube](https://…)` уходил в чат сырым — со скобками и голым
    адресом во всю ширину экрана. Теперь адрес прячется под надписью.
    ⚠️ Разбор идёт по УЖЕ экранированному тексту намеренно: «&» в адресе к
    этому моменту стал «&amp;», а атрибуту href именно это и нужно.
    """
    # quote=False: Telegram требует экранировать только «<», «>» и «&».
    # С кавычками текст статьи превратился бы в «&quot;Дрозд&quot;».
    out = html.escape(text, quote=False)
    out = _MD_LINK_RE.sub(_link_to_html, out)
...
def short_title(title: str, limit: int = 22) -> str:
    ...
    """
    # ЛЮБАЯ пометка в квадратных скобках в начале («[Скоро в игре]»,
    # «[Обновление]», «[Событие]») — так сайт называет свои новости, и в
    # очереди на одобрение все кнопки начинались с одного и того же
    # (2026-08-13, просьба Максима: срезать любые скобки, а не только
    # «[Скоро в игре]», — иначе следующая пометка вернёт ту же беду).
    # ⚠️ Срезка только для КНОПКИ: в карточке статьи заголовок остаётся
    # полным, там пометка полезна — сразу видно, что перед тобой сырой анонс.
    t = re.sub(r"^\s*\[[^\]]*\]\s*", "", title.strip())
    t = re.sub(r"^игровая\s+механика:\s*", "", t, flags=re.I)
