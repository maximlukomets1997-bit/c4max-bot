...
async def _run_proactive(bot, chat_id: int, trigger_message_id: int, trigger_text: str,
    ...
    try:
        ...
        # ⚡ ФАЙЛ УХОДИТ АКТИВНОЙ МОДЕЛИ, ЕСЛИ ОНА ЕГО ПОНИМАЕТ (21.09.2026,
        # решение Максима). Не понимает — `ask_group_proactive_media` вернёт
        # None, НЕ делая запроса, и реплику напишет она же по стенограмме:
        # разбор к этому моменту уже сохранён в архив строкой участника.
        #
        # ⚠️ ПРАВИЛО МЕНЯЛОСЬ ДВАЖДЫ. До 11.08.2026 на медиа всегда отвечала
        # активная по пересказу — он и породил шутки про ИИ над живыми людьми
        # 10 августа. Тогда решили «пусть отвечает тот, кто смотрит файл», и
        # реплику писала Gemini. Теперь — тот, кого выбрал Максим кнопкой, а
        # файл ему уходит только если он его понимает. Довод 11.08 закрыт
        # иначе: у разборщика появилось задание расшифровывать дословно.
        #
        # ⚠️ ЗАПАСНОЙ ПУТЬ ОБЯЗАТЕЛЕН И ПО ДРУГОЙ ПРИЧИНЕ: не ответила ни одна
        # модель цепочки — спрашиваем по стенограмме, и бот не онемеет из-за
        # отказа Google. Та же ветка обслуживает и «активная файл не
        # понимает»: снаружи это неотличимо и обрабатывается одинаково.
        answer = None
        if media_b64:
