...
def generate_for_article(article: dict, per_article: int = None) -> tuple[list, str]:
    ...
    #
    # ⚠️⚠️ ИМЕННО ПОЭТОМУ ЗДЕСЬ СТОИТ `thinking_override=True` (29.08.2026).
    # В тот день у каждого провайдера появилась своя кнопка глубины раздумий,
    # и без этого рычага положение «Выкл», выставленное ради быстрых ОТВЕТОВ
    # в чате, молча отключило бы размышления и в СБОРКЕ ВОПРОСОВ — то есть
    # само собой вернуло бы откаченную правку v3.90. Кнопка обещана владельцу
    # как управление ответами бота; сборка вопросов ответом не является.
    # `True` означает «думай, как умеешь»: на Gemini берётся начальный уровень,
    # у DeepSeek — ступень провайдера по умолчанию, у Qwen потолок не ставится.
