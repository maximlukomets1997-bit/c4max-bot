...
#
#  ЧТО ДЕЛАЕТ. Спрашивает у alerts.in.ua список действующих тревог и отвечает
#  на один вопрос: объявлена ли сейчас тревога у нас. «У нас» — это область
#  целиком ЛИБО отдельно Днепр (город, громада, район); так решил Максим,
#  потому что Днепропетровская область чаще всего объявляет тревогу целиком,
#  и ловить только город значило бы пропускать настоящие тревоги.
#
...
from config import (AIRALERT_API_URL, AIRALERT_OBLAST_UID, AIRALERT_TITLE_MARKERS,
                    ALERTS_TOKEN)
...
def can_watch() -> bool:
    """Настроен ли токен. Нет — вся функция молчит, а кнопка честно об этом
    говорит вместо того, чтобы включаться и ничего не делать."""
