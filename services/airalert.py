...
def is_city_level(place: str) -> bool:
    """
    Это место — сам Днепр (город, громада, район), а не область целиком?

    Различаются они по написанию: у области в названии «ДніпроПЕТРОвська»,
    у городских мест — «Дніпровський», «Дніпровська міська…», «м. Дніпро».
    Поэтому городским считается всё наше, где НЕТ куска «дніпропетров».
    Способ грубый, зато не требует справочника кодов и одинаково работает
    на обоих источниках.
    """
    return AIRALERT_OBLAST_MARKER not in (place or "").lower()


def scope_title(places: list[str]) -> str:
    """
    Заголовок сообщения о тревоге: «ДНЕПР» или «ВСЯ ОБЛАСТЬ» (2026-07-30,
    решение Максима — «оставить областные, но подписывать яснее»).

    ⚠️ «ВСЯ ОБЛАСТЬ» — это НЕ «где-то в области, меня не касается»: когда
    тревогу объявляют на всю область, Днепр в неё ВХОДИТ и сирена в городе
    звучит тоже. Именно поэтому такие тревоги владельцу приходят наравне
    с городскими, а заголовок нужен лишь чтобы с одного взгляда понимать,
    насколько широко объявлено. Не превращать этот заголовок в фильтр:
    выбросив областные тревоги, бот замолчит на части настоящих.
    Тревоги по ЧУЖИМ районам (Кривой Рог, Никополь) сюда не попадают вовсе —
    их отсекает `_is_ours` ещё при разборе ответа.

    ⚠️ На ЗАПАСНОМ источнике (без ключа) заголовок всегда «ВСЯ ОБЛАСТЬ»:
    мельче областей он ничего не знает. Это правда, а не ошибка подписи.
    """
    return "ДНЕПР" if any(is_city_level(p) for p in places) else "ВСЯ ОБЛАСТЬ"
