...
async def _handle(update, context) -> None:
    ...
    # Обновляет название и время группы в списке своих. НОВОЙ группы отсюда
    # не появится: события чужих групп сюда не доходят — их останавливает
    # заслон services/group_guard.py, он же единственный вносит группу в список.
    try:
