...
def seconds_to_next_hour(now: datetime = None) -> float:
    ...
    проснуться уже В НОВОМ часе, а не за миг до него). Полночь — тоже начало
    часа, поэтому суточный отчёт этот же расчёт обслуживает.

    ⚠️ ПОЧЕМУ ИМЕННО «ДО НАЧАЛА ЧАСА», А НЕ «ЧЕРЕЗ ЧАС» (31.08.2026). Раньше
    цикл расписания спал ровно 3600 секунд, и точка пробуждения прилипала
    к МИНУТЕ ЗАПУСКА БОТА: поднялся в 06:46 — просыпается в 07:47, 08:47,
    12:47. Вопрос дня, назначенный на 12:00, уходил в 12:47 (найдено по
    логам: старт 06:46:50 → отправки 12:47:06 и 18:47:07). А поскольку бот
    перезапускается на каждой выкатке, опоздание всякий раз новое — от нуля
    до часа. Отсюда жалоба Максима «вопрос приходит когда попало».

    ⚠️ Нижняя граница 30 секунд — защита от холостой прокрутки: без неё
    пробуждение в 12:59:58 дало бы паузу в пару секунд, а старт ровно
    в :00:00 — расчёт впритык. Лучше проснуться на полминуты позже, чем
    крутить цикл вхолостую.

    Параметр `now` нужен проверкам (selftest подставляет разные моменты);
    в бою не передаётся — берётся киевское «сейчас».
    """
    now = now or kyiv_now()
    nxt = (now + timedelta(hours=1)).replace(minute=0, second=0, microsecond=0)
    return max(30.0, (nxt - now).total_seconds() + 5)
