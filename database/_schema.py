...
def _create_schema(conn):
    ...
    conn.executescript("""
        ...
        -- СВОИ группы — где боту разрешено работать (с 15.09.2026): новую
        -- строку кладёт только заслон чужих групп services/group_guard.py
        -- (добавил владелец или он нажал «Остаться»). Отсюда же вопрос дня,
        -- дайджест и карточка пользователя (где выдавать мут/кик/бан).
        -- Название обновляется на каждое сообщение своей группы.
