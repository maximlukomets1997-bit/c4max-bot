...
def set_quiz_stats(user_id: int, correct: int, attempts: int,
                   username: str = "") -> None:
    """
    Ставит счёт викторины ОДНОМУ человеку (кнопка «🎯 Счёт викторины» в
    карточке участника, 04.09.2026, просьба Максима). Раньше счёт правился
    только руками в базе на сервере или обнулялся сразу всем.

    Числа кладутся КАК ДАНЫ — проверять их здесь нечем: «верных не больше
    попыток» и потолок живут там же, где надпись на кнопке
    (handlers/admin/panel_users.py::_set_quiz_score), общие для бота и сайта.

    Имя пишется ТОЛЬКО новой строке: у игравшего оно уже есть и обновляется
    самой игрой на каждом ответе. Ручная правка счёта — не повод переименовать
    человека в дайджесте.

    ⚠️ ВМЕСТЕ СО СЧЁТОМ ПОДВИГАЕТСЯ НЕДЕЛЬНЫЙ СНИМОК ДАЙДЖЕСТА (settings
    `group_digest_quiz`) — та же засада, из-за которой его трогает и
    `reset_all_quiz_stats`. Снимок хранит «сколько было верных ответов на
    начало недели», а неделя считается разницей с ним, поэтому ручная правка
    без него врала бы В ОБЕ СТОРОНЫ:
      • подняли счёт — дайджест объявит В ГРУППУ «за неделю +115 ответов»,
        которых человек не давал;
      • опустили — разница уйдёт в минус, и дайджест будет писать про него
        «0 за неделю», пока он не переиграет старые цифры.
    Ручная правка не заработана на этой неделе, поэтому в снимке человек
    сразу оказывается на новом числе.

    ⚠️ Снимок трогаем, ТОЛЬКО если он уже есть. Отсутствие ключа значит
    «дайджест ещё ни разу не отправлялся», и первый его выпуск намеренно
    считает всё накопленное — создать снимок раньше времени значит отнять
    этот первый счёт у всех остальных.
    """
    with _lock:
        conn = _get_connection()
        conn.execute(
            ...
            (user_id, username or "", int(correct), int(attempts)),
        )

        row = conn.execute("SELECT value FROM settings WHERE key='group_digest_quiz'").fetchone()
        if row and row["value"]:
            try:
                snapshot = json.loads(row["value"]) or {}
                snapshot[str(user_id)] = int(correct)
                conn.execute("UPDATE settings SET value=? WHERE key='group_digest_quiz'",
                             (json.dumps(snapshot),))
            except (TypeError, ValueError) as e:
                # Битый снимок — не повод срывать саму правку счёта: дайджест
                # переживёт один выпуск, а счёт человеку нужен сейчас.
                logger.warning("⚠️ Недельный снимок дайджеста не разобрался, "
                               "счёт викторины поставлен без него: %s", e)
        conn.commit()

    logger.info("🎯 Счёт викторины у %s: %d верных из %d", user_id, correct, attempts)
