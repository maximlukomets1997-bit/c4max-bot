...
#    known_chats    — СВОИ группы: где боту разрешено работать. С 15.09.2026
#                     это список одобренных владельцем (services/group_guard.py):
#                     группы, которой здесь нет, бот не слушает вовсе. Отсюда
#                     же вопрос дня, дайджест и список групп в карточке участника
#
#  Что отсюда читают:
...
#    services/group_guard.py  — заслон чужих групп: единственный, кто вносит
#                               НОВУЮ группу в known_chats, и он же её убирает
#
...
#
#  ⚠️ АРХИВ СООБЩЕНИЙ НЕ ПОКРЫТ selftest НИ ОДНИМ ВЫЗОВОМ. Правка его функций
#  страховки не имеет и проверяется только руками: контекст режима «Сам в
#  разговор», недельный дайджест, список групп в карточке участника.
#  Покрыт только список своих групп — remember_chat, is_known_chat и
#  forget_chat гоняет проверка «чужие группы» (15.09.2026).
# ───────────────────────────────────────────────
...
# ─────────────────────────────────────────────
#  Известные группы — список СВОИХ групп (services/group_guard.py)
# ─────────────────────────────────────────────

def remember_chat(chat_id: int, title: str = "") -> None:
    """
    Запоминает группу, где работает бот. НОВУЮ строку сюда кладёт только
    заслон чужих групп (services/group_guard.py) — бота добавил владелец или
    он нажал «✅ Остаться». Архиватор (collect_group_message) зовёт эту
    функцию на каждое сообщение, но сообщения чужих групп до него не доходят,
    и для своей группы это лишь обновление названия и времени: переименование
...
def is_known_chat(chat_id: int) -> bool:
    """
    Своя ли группа. Зовётся заслоном на КАЖДОЕ обновление из группы, поэтому
    внутри один запрос по первичному ключу и больше ничего.
    """
    with _lock:
        conn = _get_connection()
        row = conn.execute("SELECT 1 FROM known_chats WHERE chat_id = ?",
                           (chat_id,)).fetchone()
    return row is not None


def forget_chat(chat_id: int) -> bool:
    """
    Убирает группу из своих: бота из неё удалили, он вышел сам или группа
    переехала на новый номер. True — строка была.
    """
    with _lock:
        conn = _get_connection()
        cur = conn.execute("DELETE FROM known_chats WHERE chat_id = ?", (chat_id,))
        conn.commit()
