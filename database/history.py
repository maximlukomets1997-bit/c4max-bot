...
def init_db():
    """Создаёт таблицы при первом запуске. Вызывается один раз из main.py."""
    with _lock:
        ...
        # Добавляем колонку model_name в существующую БД, если её там нет
        try:
        ...
        # Графа «кто снял мут» в журнале модерации — добавляем в существующую
        # БД, если её там нет (у старых записей останется пустой).
        try:
            conn.execute("ALTER TABLE moderation_log ADD COLUMN admin_name TEXT")
            conn.commit()
        except sqlite3.OperationalError:
            # Колонка уже есть
            pass

        # Накопительный учёт токенов на пару (chat_id, user_id).
        # Добавляем колонки в существующую БД, если их там нет.
        for _ddl in (
        ...
        # Имя и ник в личном деле — для списка пользователей в админ-панели.
        # Раньше личное дело хранило только числовой ID, а имена лежали в архиве
        # групп, который чистится раз в 10 дней: молчащий участник превращался
        # в безымянное число. Теперь имя пишется в дело на каждое сообщение.
        for _ddl in (
        ...
        # Одноразовое заполнение списка групп из архива сообщений: до первого
        # сообщения после запуска known_chats была бы пуста, и карточка
        # пользователя не знала бы, где выдавать мут. Названия групп подтянутся
        # сами, как только в них кто-нибудь напишет.
        try:
            seeded_chats = conn.execute("SELECT COUNT(*) FROM known_chats").fetchone()[0]
            if seeded_chats == 0:
                conn.execute(
        ...
        # Одноразовый перенос статистики запросов из устаревшей user_context
        # (агрегируем по user_id). Токены ранее не писались (всегда были 0),
        # поэтому total_tokens стартует с нуля и далее считается корректно.
        try:
            seeded = conn.execute("SELECT COUNT(*) FROM user_token_usage").fetchone()[0]
            if seeded == 0:
                conn.execute(
        ...
        # Миграция: has_voice добавлен 2026-07-21 в group_messages.
        # Для существующих БД без этой колонки — ловим ошибку и молча идём дальше.
        try:
            conn.execute("ALTER TABLE group_messages ADD COLUMN has_voice INTEGER DEFAULT 0")
        except sqlite3.OperationalError:
            pass

        # Миграция: has_video добавлен 2026-07-24 (бот научился принимать видео).
        try:
            conn.execute("ALTER TABLE group_messages ADD COLUMN has_video INTEGER DEFAULT 0")
        except sqlite3.OperationalError:
            pass
