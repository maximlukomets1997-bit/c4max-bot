...
| `web/routes.py` | 765 | `ROUTES` — единственный список адресов; `build_app`, `index`, `apply`, `prompts`, `kb`, `quiz`, `users`, `user_card`, `system`, `download`, `enter`, `exit_`, `health` |
...
| `web/pages.py` | 1512 | `esc`, `page_login`, `page_denied`, `page_summary`, `page_prompts`, `page_users`, `page_user_card`, `page_kb`, `page_quiz`, `page_system`, `plain`, `current_theme` + `THEMES` (две темы оформления), `NAV` (список разделов верхней полосы), `css_version` (отпечаток оформления против кэша браузера) |
