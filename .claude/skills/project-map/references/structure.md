...
| `services/group_guard.py` | 459 | 3 | 🚪 заслон от чужих групп (15.09.2026): `gate` (группа −3, ПЕРВЫМ среди обработчиков — обновление из группы, которой нет в `known_chats`, дальше не идёт), `on_my_chat_member` (бота добавили/удалили), `ask_owner` и `request_keyboard` (вопрос владельцу с кнопками `grp:stay:`/`grp:leave:`), `handle_group_callback` (кнопки, после гейта прав), `forget_group`. ⚠️ Сбой проверки ПРОПУСКАЕТ, а не запирает. ⚠️ О каких группах уже спросили — в settings `group_guard_pending`, не в памяти. ⚠️ «Состоит ли бот в группе» решает `greeter._is_in` — второго правила не заводить |
