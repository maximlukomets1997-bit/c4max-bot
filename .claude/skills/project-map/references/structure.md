...
Снимок от 2026-08-16 (после уборки неиспользуемых импортов — 86 штук в 16
файлах), таблица целиком пересобрана скриптом и сверена с кодом. Пересобрать:
...
| `handlers/quiz.py` | 325 | 3 | `send_quiz_question`, `cmd_quiz`, `cmd_rank`, `send_rank_panel`, `handle_poll_answer` |
...
| `handlers/admin/panel_main.py` | 444 | 4 | `send_stats_panel`, `send_api_panel`, `send_daily_report_panel`, `send_weekly_report_panel`, `cmd_stats`, `build_adm_keyboard`, `send_adm_panel`, `cmd_adm` |
...
| файл | строк | тянут | публичные имена |
|---|---:|---:|---|
| `services/gemini.py` | 2526 | 7 | `compress_newlines`, `ask_gemini`, `ask_gemini_audio`, `ask_gemini_video`, `format_news_as_colonel`, `author_brief`, `last_news_brief`, `ask_group_proactive`, `ask_group_proactive_media`, `generate_image` |
