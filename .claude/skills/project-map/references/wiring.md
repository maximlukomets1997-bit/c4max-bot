...
Снимок от 2026-08-11. Всё ниже проверено чтением кода; если правишь что-то из
...
считает зарегистрированные обработчики и группы (на 2026-08-11 — **37 в 4
группах**).
...
с ветками роутера (на 2026-08-11 — **208 кнопок, 34 точных ветки + 13 по
приставке**).
...
Один файл — `database/history.py` (2728 строк, 110 публичных функций),
одно соединение на процесс, доступ под общим замком `_lock`.

Схема создаётся в `_create_schema`; на 2026-08-11 — **24 таблицы**:
...
`config.py` (1016 строк, ~106 констант) — читают 34 модуля. Значения
