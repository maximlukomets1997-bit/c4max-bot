...
идёт вовсе (`ApplicationHandlerStop`). Два обработчика, ПОРЯДОК ВАЖЕН:

- `ChatMemberHandler(on_my_chat_member, MY_CHAT_MEMBER)` — сменился статус
  САМОГО бота: добавил владелец → группа своя; добавил чужой → вопрос
  владельцу; удалили → группа уходит из списка и из памяти вопроса дня;
- `TypeHandler(Update, gate)` — сам заслон.

⚠️ В одной группе срабатывает только первый подходящий обработчик: поставь
заслон впереди — он съест событие «бота добавили», и вопрос не уйдёт. Оба
обязаны быть блокирующими: в неблокирующем `ApplicationHandlerStop` не
действует. Всё это ловит `selftest.py::check_group_guard` — он гоняет
НАСТОЯЩУЮ `setup_handlers` на поддельном боте.
