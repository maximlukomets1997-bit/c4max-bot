...
_NOISY_LOGGERS = {
    ...
    # Каждый SQL-запрос логируется на DEBUG (см. database._core._LoggingConnection).
    ...
    #
    # ⚠️ ГЛУШИМ ВЕСЬ ПАКЕТ «database», А НЕ ОТДЕЛЬНЫЙ ФАЙЛ (02.09.2026). Раньше
    # здесь стояло "database.history" — по имени единственного тогда файла. Он
    # разрезается на части, и логирующее соединение уехало в database/_core.py;
    # прежняя строка перестала бы на него действовать, и SQL-отладка молча
    # хлынула бы в консоль и в файл лога. Настройка на имя пакета накрывает всех
    # его потомков разом — и тех, что появятся дальше по разрезу.
