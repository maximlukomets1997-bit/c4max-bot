...
#                           о расходах владельцу в личку; там же, следом за
#                           отчётом, уходит ночная копия базы (nightly_backup)
    ...
    while True:
        try:
            ...
            if active and not was_active:
                ...
                where = ", ".join(places[:4]) if places else "Дніпропетровська область"
                # Заголовок говорит, НАСКОЛЬКО ШИРОКО объявлено (решение Максима
                # 2026-07-30): «ДНЕПР» — тревога по самому городу, громаде или
                # району; «ВСЯ ОБЛАСТЬ» — объявлена целиком по области.
                scope = airalert.scope_title(places)
                head = f"🚨 <b>ВОЗДУШНАЯ ТРЕВОГА · {scope}</b>"
                if first_look:
                    # Тумблер включили посреди тревоги (или бот не работал,
                    # когда её объявили) — честно говорим, что она уже идёт.
                    head += " <i>(уже идёт)</i>"
                # ⚠️ У областной тревоги поясняющая строка ОБЯЗАТЕЛЬНА: без неё
                # заголовок «ВСЯ ОБЛАСТЬ» читается как «где-то в области, меня
                # не касается», а на деле Днепр в неё входит и сирена в городе
                # звучит тоже. Ради этой ясности областные тревоги и оставлены
                # в рассылке — не убирать строку вместе с ними.
                note = ("\n<i>Днепр в том числе — сирена звучит и в городе</i>"
                        if scope == "ВСЯ ОБЛАСТЬ" else "")
                await _tell_owner(
                    f"{head}\n"
                    "───────────────────────────\n"
                ...
                logger.info("🚨 Объявлена воздушная тревога (%s): %s", scope, where)
