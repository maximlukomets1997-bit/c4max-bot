...
async def post_init(application):
    ...
    application.bot_data["background_tasks"] = [
        ...
        # Сверка остатка на счету с платформой провайдера раз в 10 минут
        # (14.09.2026). Первая — через 15 секунд после запуска: иначе экраны
        # до 10 минут показывали бы собственную оценку бота, а она занижена на
    ...
    # Отдельно — уведомление, которое ПРОВИСЕЛО СВОЁ (03.09.2026, срок в
    # config.UPDATE_NOTICE_TTL_SEC). Проверка нужна именно здесь, а не только
    # в цикле самообновления: бот мог простоять выключенным дольше срока, и
    # тогда уведомление устарело ещё до его пробуждения.
    # ⚠️ Это НЕ дубль строки выше: та убирает уведомление ПРОШЛОЙ сборки и о
    # сроках не знает, эта — уведомление про текущий код, просто отвисевшее.
    try:
        from jobs import drop_expired_notice
