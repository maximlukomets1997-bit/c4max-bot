...
def _restart_self() -> None:
    ...
    В Docker новая копия не запускается: контейнер просто завершается, и его
    поднимает заново политика перезапуска Docker.
    """
    if IS_DOCKER:
        logger.info("🚀 Перезапуск в Docker: контейнер завершается, его поднимет политика перезапуска")
        return
...
def main():
    ...
    if not IS_DOCKER:
        ...
        logger.info("🚀 Бот запущен внутри Docker-контейнера — локальная защита PID отключена")
    # ────────────────────────────────────────────────────────────────────────
